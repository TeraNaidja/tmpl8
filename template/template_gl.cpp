...
void Shader::Init(const char* vfile, const char* pfile)
{
	...
	FATALERROR_IF(vsText.empty(), "File %s not found", vfile);
	FATALERROR_IF(fsText.empty(), "File %s not found", pfile);
...
void Tmpl8::_CheckGL(const char* f, int l)
{
	GLenum error;
...
GLuint Tmpl8::CreateVBO(const GLfloat* data, const uint size)
{
	GLuint id;
	glGenBuffers(1, &id);
...
void Tmpl8::BindVBO(const uint idx, const uint N, const GLuint id)
{
